...
export default function createBackground(pixiApp) {
  ...
  for (let i = 0; i < starAmount; i++) {
    ...
    pixiApp.stage.addChild(star.sprite);
    stars.push(star);
  }
  ...
  pixiApp.ticker.add(delta => {
    ...
    for (let i = 0; i < starAmount; i++) {
      ...
      star.sprite.x =
        star.x * (fov / z) * pixiApp.renderer.screen.width +
        pixiApp.renderer.screen.width / 2;
      star.sprite.y =
        star.y * (fov / z) * pixiApp.renderer.screen.width +
        pixiApp.renderer.screen.height / 2;

      // Calculate star scale & rotation.
      const dxCenter = star.sprite.x - pixiApp.renderer.screen.width / 2;
      const dyCenter = star.sprite.y - pixiApp.renderer.screen.height / 2;
